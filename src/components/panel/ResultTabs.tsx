  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="flex border-b border-gray-100">
        {TABS.map(tab => (
          <button
            ...
            className={cn(
              'flex-1 py-3 px-2 text-center text-sm font-medium transition-all border-b-2',
              activeTab === tab.id
      ...
      <div className="p-5">
        ...
        {activeTab === 'catastral' && (
          <div>
            {catastral && catastral.torres.length > 0 ? (
                ...
                {catastral.torres.map((torre, i) => {
                  ...
                  return (
                    <div key={i} className="p-4 bg-teal-50 rounded-xl">
                      <h4 className="font-semibold text-sm mb-2">{project || 'Edificio'}</h4>
              ...
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-sm">Sin datos catastrales para esta dirección</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'zona' && (
          <div>
            {medianZone && medianZone.leads_cierres > 0 ? (
              <div className="space-y-4">
                <div className="p-4 bg-purple-50 rounded-xl">
                  <div className="text-3xl font-bold text-purple-700 mb-1">{medianZone.leads_cierres}</div>
                  <p className="text-sm text-gray-600">inmuebles comprados por Habi en tu zona <span className="text-gray-400">(últimos 12 meses)</span></p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-gray-50 rounded-xl">
                    <div className="text-xl font-bold text-gray-800">{medianZone.leads_cierres_desistimiento}</div>
                    <p className="text-xs text-gray-500">ventas estimadas en la zona</p>
                  </div>
                  {medianZone.ultimo_cierre && (
                    <div className="p-3 bg-gray-50 rounded-xl">
                      <div className="text-sm font-bold text-gray-800">{medianZone.ultimo_cierre.substring(0, 10)}</div>
                      <p className="text-xs text-gray-500">última compra de Habi</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="text-center py-8 text-gray-400">
                <div className="text-3xl mb-2">📊</div>
                <p className="text-sm">Aún no tenemos actividad registrada en esta zona</p>
              </div>
            )}
          </div>
        )}
